...
fn parse_rtd_leads(s: &str) -> Result<RTDLeads, String> {
    match s.to_lowercase().as_str() {
        ...
        _ => Err(format!(
            "Invalid leads: {}. Must be Two/2, Three/3, or Four/4.",
            s
        )),
    }
}
...
fn parse_filter_hz(s: &str) -> Result<FilterHz, String> {
    match s.to_lowercase().as_str() {
        ...
        _ => Err(format!(
            "Invalid filter: {}. Must be Fifty/50 or Sixty/60.",
            s
        )),
    }
}
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
            ...
            match key {
                "cs-pin" => {
                    cli_cs_pin = Some(
                        value
                            .parse()
                            .map_err(|e| format!("Invalid --cs-pin '{}': {}", value, e))?,
                    );
                }
                "leads" => {
                    cli_leads = Some(
                        parse_rtd_leads(value)
                            .map_err(|e| format!("Invalid --leads '{}': {}", value, e))?,
                    );
                }
                "filter" => {
                    cli_filter = Some(
                        parse_filter_hz(value)
    ...
    let cs_pin_str = cli_cs_pin
        .map(|p| p.to_string())
        .or_else(|| env::var("MAX31865_CS_PIN").ok())
        .unwrap_or_else(|| "24".to_string());
    let cs_pin: u8 = cs_pin_str
        .parse()
        .map_err(|e| format!("Invalid CS_PIN '{}': {}", cs_pin_str, e))?;

    let leads_str = cli_leads
        .map(|l| match l {
    ...
    let leads =
        parse_rtd_leads(&leads_str).map_err(|e| format!("Invalid LEADS '{}': {}", leads_str, e))?;

    let filter_str = cli_filter
        .map(|f| match f {
    ...
    let filter = parse_filter_hz(&filter_str)
    ...
    println!(
        "It checks initial room temp, monitors cooling in the bath, and recovery after removal."
    );
    println!("Safety: Ensure probe is fully submerged (stir ice/water mix); keep water away from electronics.");
    println!(
        "Expected: Temps stabilize near 32°F in bath (±2°F ok), rise to 60°F+ after ~1-2 min."
    );
    println!("If temps don't change or faults occur, check wiring/probe.");
    println!(
        "Config: CS pin={}, {} leads, {} Hz filter",
        cs_pin,
        leads as u8,
        match filter {
            FilterHz::Fifty => 50,
    ...
    println!(
        "(From CLI/env vars: --cs-pin/--leads/--filter or MAX31865_*; defaults used if unset)"
    );
    println!();
    ...
    let raw = match raw_result {
        ...
        Err(e) => {
            ...
            eprintln!(
                "Raw read failed: {:?}. Fault status: 0x{:02X} -> {:?}",
                e,
                status,
    ...
    if !(40.0..=110.0).contains(&temp_f) {
        eprintln!(
            "Initial temp {:.1}°F out of range (expected 40-110°F).",
            temp_f
        );
        eprintln!(
            "Raw: 0x{:04X} (LSB fault: {}) | Resistance: {:.2} ohms | Temp: {:.1}°C",
            raw,
            raw & 1,
            resistance,
            temp_c
        ...
        eprintln!(
            "If raw ~0x0000 or resistance ~0 ohms: Short circuit (check for solder bridges)."
        );
    ...
    println!(
        "Initial temp: {:.1}°F ({:.1}°C) - OK to proceed.",
        temp_f, temp_c
    );
    ...
    loop {
        if consecutive_errors >= 5 {
            return Err(
                "Too many consecutive read errors (5+). Aborting test—check hardware.".into(),
            );
        }
        ...
        match read_temp_with_faults(&mut reader, "Bath") {
            Ok(temp_f) => {
                ...
                println!(
                    "Elapsed: {}s | Temp: {:.1}°F ({:.1}°C) | Min so far: {:.1}°F",
                    elapsed, temp_f, temp_c, min_temp_f
                );

                // Simple stuck check: Warn if no change for 10s (approx 10 reads)
                if (temp_f - last_temp_f).abs() < 0.1 && elapsed > 10 && !stuck_warning {
                    println!(
                        "Warning: Temperature not changing—stir bath or check for stuck reads."
                    );
                    stuck_warning = true;
                }
                last_temp_f = temp_f;
    ...
    let reason = if bath_start.elapsed().as_secs() >= 300 {
        "timeout"
    } else {
        "threshold hit"
    };
    println!(
        "\nBath phase done ({}). Min temp: {:.1}°F ({:.1}°C)",
        reason, min_temp_f, bath_min_c
    );
    if min_temp_f > 34.0 {
    ...
    loop {
        ...
        match read_temp_with_faults(&mut reader, "Recovery") {
            Ok(temp_f) => {
                ...
                let temp_c = (temp_f - 32.0) * 5.0 / 9.0;
                println!(
                    "Elapsed: {}s | Temp: {:.1}°F ({:.1}°C) | Max so far: {:.1}°F",
                    elapsed, temp_f, temp_c, max_temp_f
                );
    ...
    let reason = if recovery_start.elapsed().as_secs() >= 300 {
        "timeout"
    } else {
        "threshold hit"
    };
    println!(
        "\nRecovery phase done ({}). Final temp: {:.1}°F ({:.1}°C)",
        reason, final_temp_f, final_temp_c
    );
    println!();
    ...
    println!(
        "Recovery final: {:.1}°F (expected near initial)",
        final_temp_f
    );
    if !(30.0..=34.0).contains(&min_temp_f) {
        println!(
            "Potential issue: Bath temp off from 32°F. Recheck setup or compare to reference code."
        );
    } else {
...
fn read_temp_with_faults(
    reader: &mut RTDReader,
    phase: &str,
                ...
                match reader.read_fault_status() {
                    Ok(status) if status != 0 => {
                        ...
                        match reader.get_temperature() {
                            Ok(t) => Ok(t),
                            Err(retry_e) => {
                                Err(format!("Retry failed after MAX fault: {:?}", retry_e))
                            }
                        }
                    }
                    _ => Err(format!(
                        "MAX fault during read (status unavailable): {:?}",
                        e
                    )),
                }
            } else {
