...
mod private {
    ...
    #[derive(Debug)]
    pub enum Error {
        /// Error reading from the Max31865 chip registers
        SpiErrorRead,
...
// Public simplified API
pub use crate::rtd_reader::{RTDReader};

// Internal low-level driver (not exposed publicly)
mod low_level {
    use super::*;
...
pub mod rtd_reader {
    use super::private::{Max31865, SensorType, FilterMode as InternalFilterMode, Error as InternalError};
    ...
    impl RTDReader {
        ...
        pub fn new(cs_pin: u8, leads: super::RTDLeads, filter: super::FilterHz) -> Result<Self, super::RtdError> {
            let gpio = Gpio::new().map_err(|e| super::RtdError::Init(format!("GPIO init failed: {}", e)))?;
            let ncs = gpio.get(cs_pin).map_err(|e| super::RtdError::Init(format!("NCS pin {} invalid: {}", cs_pin, e)))?.into_output_high();
            let spi = Spi::new(Bus::Spi0, SlaveSelect::Ss0, 1_000_000, SpiMode::Mode3)
                .map_err(|e| super::RtdError::Init(format!("SPI init failed: {}", e)))?;

            let mut inner = Max31865::new(spi, ncs).map_err(|e| super::RtdError::Init(match e {
                InternalError::PinError => "NCS pin setup failed".to_string(),
                _ => "MAX31865 init failed".to_string(),
            }))?;

            let sensor_type = match leads {
                super::RTDLeads::Two | super::RTDLeads::Four => SensorType::TwoOrFourWire,
                super::RTDLeads::Three => SensorType::ThreeWire,
            };

            let internal_filter = match filter {
                super::FilterHz::Fifty => InternalFilterMode::Filter50Hz,
                super::FilterHz::Sixty => InternalFilterMode::Filter60Hz,
            };

            inner.configure(true, true, false, sensor_type, internal_filter)
                .map_err(|e| super::RtdError::Init(format!("Configure failed: {:?}", e)))?;

            Ok(RTDReader { inner })
        }

        /// Read temperature in °C as f64 (PT100 lookup).
        pub fn get_temperature(&mut self) -> Result<f64, super::RtdError> {
            self.inner.read_temperature().map_err(map_internal_error)
        }

        /// Read resistance in ohms as f64.
        pub fn get_resistance(&mut self) -> Result<f64, super::RtdError> {
            self.inner.read_resistance().map_err(map_internal_error)
        }

        /// Read temperature as scaled integer (degrees Celsius * 100).
        pub fn read_temp_100(&mut self) -> Result<i32, super::RtdError> {
            self.inner.read_default_conversion().map_err(map_internal_error)
        }

        /// Read resistance as scaled integer (ohms * 100).
        pub fn get_ohms_100(&mut self) -> Result<u32, super::RtdError> {
            self.inner.read_ohms().map_err(map_internal_error)
        }

        /// Read raw RTD value (u16, for testing/low-level).
        pub fn get_raw_data(&mut self) -> Result<u16, super::RtdError> {
            self.inner.read_raw().map_err(map_internal_error)
        }

        /// Check if an error is a MAX31865 fault (RtdError::Fault variant).
        pub fn is_max_fault(&self, e: &super::RtdError) -> bool {
            matches!(e, super::RtdError::Fault(_))
        }

        /// Read fault status (u8 from reg 0x07; auto-clears).
        pub fn read_fault_status(&mut self) -> Result<u8, super::RtdError> {
            self.inner.read_fault_status().map_err(map_internal_error)
        }

        /// Clear any latched faults (no-op if none).
        pub fn clear_fault(&mut self) -> Result<(), super::RtdError> {
            self.inner.clear_fault().map_err(|e| super::RtdError::Read(match e {
    ...
    fn map_internal_error(e: InternalError) -> super::RtdError {
        match e {
            InternalError::SpiErrorRead | InternalError::SpiErrorWrite | InternalError::SpiErrorTransfer | InternalError::PinError => {
                super::RtdError::Read("SPI/GPIO transfer failed".to_string())
            }
            InternalError::ConfigError => super::RtdError::Init("Invalid configuration".to_string()),
            InternalError::MAXError => super::RtdError::Fault(0),  // Placeholder; call read_fault_status() for real status
        }
    }
}